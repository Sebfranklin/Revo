...
module.exports = async (req, res) => {
    ...
    try {
        // --- Schema Setup (Ensures table exists on first run) ---
        await sql`
        ...
        if (!history || !Array.isArray(history) || !system_prompt || !userId) {
            return res.status(400).json({ error: 'Valid history, system_prompt, and userId are required.' });
        }
