document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.lucide && typeof lucide.createIcons === 'function') {
        try {
            ...
            console.warn('Initial lucide.createIcons failed:', err);
        }
    }
    ...
    let orbConversationHistory = []; // Separate history for AI Orb (session-only, not saved)
    ...
    if (SpeechRecognition) {
        ...
        recognition.onresult = async (event) => {
            ...
            if (transcript) {
                const aiResponse = await fetchOrbAIResponse(transcript);
    ...
    function showAIOrb(service, action) {
        orbConversationHistory = []; // Clear orb-specific history instead of main chat history
        if (!SpeechRecognition) {
        ...
        fetchOrbAIResponse(initialPrompt).then(speakText);
    }
    ...
    if (!userId) {
        userId = `trixel-user-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`;
        ...
        try {
            ...
            if (conversationHistory.length > 0) {
                conversationHistory.forEach(msg => {
                    ...
                    const content = msg.parts && Array.isArray(msg.parts) && msg.parts[0] ? msg.parts[0].text : '';
    ...
    function showAIChat() {
        ...
        loadConversation(); // Load history when opening the chat
    ...
    clearChatBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch(`/api/chat?userId=${userId}`, { method: 'DELETE' });
            if (!response.ok) {
                throw new Error('Failed to clear history on the server.');
            }
            // On success, clear local state and UI
            conversationHistory = []; 
            loadConversation(); // This will now show the welcome message
        } catch (error) {
            console.error('Error clearing conversation:', error);
            alert('Could not clear the conversation. Please try again.');
        }
    });
        ...
        try {
            const response = await fetch(API_URL, {
                ...
                body: JSON.stringify({
                    ...
                    userId: userId // Include the user ID for persistence
                })
            });
    ...
    // New function for AI Orb (session-only, not saved to database)
    async function fetchOrbAIResponse(prompt) {
        // Add the new user prompt to the orb-specific history in Gemini's format
        orbConversationHistory.push({ role: "user", parts: [{ text: prompt }] });
        ...
        try {
            const response = await fetch(API_URL, {
                ...
                body: JSON.stringify({
                    history: orbConversationHistory,
                    system_prompt: BUSINESS_CONTEXT,
                    userId: null // Crucially: null userId means do NOT save to database
                })
            });
            ...
            console.error("Error fetching AI Orb response from backend:", error);
            return "I'm sorry, I'm having trouble connecting to my brain right now. Please try again in a moment.";
        }
    }
...
function updatePlayPauseIcon() {
    ...
    if (!icon) {
        // If icon doesn't exist, create it (fallback for first load)
        ...
        playPauseBtn.innerHTML = ''; // Clear
        playPauseBtn.appendChild(newIconEl);
    } else {
        // If icon exists, just change its attribute
        const newIconName = AudioManager.getState().music === 'playing' ? 'pause' : 'play';
...
// =================================================================
    ...
    // =================================================================
    // START: REVIEW SLIDESHOW & SUBMISSION
    // =================================================================
    ...
    let reviewSlideshowInterval = null;
    ...
    function showReview(index) {
        if (!loadedReviews || loadedReviews.length === 0) return;
        
        const review = loadedReviews[index];
        const reviewCard = `
    ...
    function renderReviews(reviews) {
        ...
        if (reviewSlideshowInterval) {
            clearInterval(reviewSlideshowInterval);
            reviewSlideshowInterval = null;
        }
        ...
        if (!reviews || reviews.length === 0) {
            reviewsContainer.innerHTML = `<div class="text-center text-[var(--text-secondary)] p-8 col-span-full">No reviews yet. Be the first to share your experience!</div>`;
            return;
        }
        ...
        // Start slideshow if there are multiple reviews
        if (loadedReviews.length > 1) {
            reviewSlideshowInterval = setInterval(() => {
    ...
    async function fetchAndRenderReviews() {
        try {
            ...
            reviewsContainer.innerHTML = `<div class="md:col-span-2 text-center text-red-500 p-8">Could not load reviews at this time.</div>`;
        }
    }
    
    if (starRatingContainer && ratingValueInput && reviewForm) {
        const maxStars = 5;
        ...
        fetchAndRenderReviews(); // Initial load of reviews
    }
    if (reviewForm) { // Ensure reviewForm exists before adding listener
        reviewForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (response.ok) {
                    alert(result.message); // Show success message from the API
                    reviewForm.reset();
                    ratingValueInput.value = "0";
                    currentRating = 0;
                    updateStarsUI(0);
                    fetchAndRenderReviews(); // Refresh the reviews list
                } else {
    ...
    // =================================================================
    // END: REVIEW SLIDESHOW & SUBMISSION
    // =================================================================
